...
DST_GRAPH = OFFICIAL_GRAPH_PATHS["entertainment"]
...
NODE_IDS_TO_DELETE = {
    ...
    "f27969dd-cdcd-43da-9e75-328e4546239c",  # 发票报销年份检查 W40
...
def _build_taxi_invoice_serial_check_node() -> dict:
    ...
    return _make_decision_table(
        node_id=node_id,
        name="出租车发票连号检查",
        input_field="isEntertainmentTaxiInvoiceSerialClean",
        input_name="出租车发票号码是否存在连号",
        rules=rules,
        output_path="taxi_invoice_serial_result",
...
# 判定边界（必须遵守）
下面列出的是公司制度的“禁止项目负面清单”，不是“允许项目白名单”：
- 只有在发票项目的真实语义明确属于禁止项目时才拒绝；
- 不要要求项目名称逐字命中示例，也不要因为没有出现在示例中就拒绝；
- 发票内容只要没有明确命中禁止项目，或语义无法确认，默认通过；
- 普通餐饮、酒水、食品及其他正常招待消费，不因名称陌生、表达简略或未列出示例而拒绝。
...
# 语义判断要求（非常重要）
这是语义审核，不是字符串包含判断。请先对项目名称做必要的语义归一化（忽略多余空格、标点、词序变化、常见 OCR 差异和同义表达），再判断完整短语和实际商品含义：
- “黄金针菇”是蔬菜，不是黄金，必须通过；
- “黄金饰品”“黄金手链”“黄金摆件”属于黄金/首饰，必须不通过；
- “珠宝首饰”“钻石珠宝”等明确属于珠宝或首饰，必须不通过；
- “茅台酒”“五粮液白酒”明确属于对应品牌，必须不通过；
- 只要项目名称明确出现品牌“茅台”或“五粮液”，无论后面是白酒、红酒、干红、系列名称或其他商品描述，均视为命中禁止清单；例如“茅台老树S15干红葡萄酒”必须返回 prohibited_item；
- 仅因出现“黄金”二字但实际是食品、植物、地名或其他普通商品（例如黄金针菇、黄金果），不能判为不通过；
- 仅有“礼品”二字不等于礼品卡；只有明确是礼品卡、充值卡或预付/储值卡等卡类/资金预存项目时才不通过；
- 仅因出现“礼品”“酒”“金”“卡”等单个字词，不能推断为禁止项目；必须结合完整语义判断；
- 项目名称含义不清、无法确认命中禁止清单时，默认通过；
- 多个项目名称中只要有一个明确命中禁止清单，就判定不通过，其他未命中项目不得被连带扣减。
...
def build_entertainment_graph(source_path: Path | str | None = None) -> dict:
    ...
    TAXI_INVOICE_SERIAL_CHECK = "ent-taxi-invoice-serial-check"
